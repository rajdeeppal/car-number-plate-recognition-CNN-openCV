...
def plate_segmentation(img_file_path):
    ...
    #imgray = cv2.imread(img_file_path , 0 )
    img = cv2.imread(img_file_path)
    print( img.shape )
    imgray = cv2.imread(img_file_path , 0 )
    ...
    for cnt in contours:
        ...
        if (w * h > area_condition1 and w * h < area_condition2 and w/h > 0.2 and h/w > 0.2):
